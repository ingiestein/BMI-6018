...
flights_data["date"] = pd.to_datetime(
    flights_data[["year", "month", "day"]]
).dt.strftime("%Y/%m/%d")
...
q_5 = (
    flights_data.groupby("date")["dep_delay"]
    .mean()
...
# q_6
q_6 = (
    flights_data.groupby("date")["arr_delay"]
    .mean()
    .sort_values(ascending=False)
    .iloc[0]
)
...
q_7 = (
    flights_data[
        ...
        & ((flights_data["origin"] == "LGA") | (flights_data["origin"] == "JFK"))
    ][["tailnum", "speed"]]
    .sort_values(ascending=False, by="speed")
    .iloc[0]
)
print("q_7: ", q_7)
...
# q_10
q_10 = weather_data_np[mask][
    :, 7
].mean()  # taking the mean of the index 7 column (humidity) and getting the mean.
print("q_10: ", q_10)
